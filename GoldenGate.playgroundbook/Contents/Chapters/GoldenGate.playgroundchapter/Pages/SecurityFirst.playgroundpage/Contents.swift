...
/*:
 ...
 ## First Things First
 In the digital world of ones and zeros, components can have only one of two states: on or off. (Technically there might also be a third _high impedance_ state, which is not relevant for these puzzles.)
 This might sound quite limiting but is actually the basis for how this very _iPad_ operates—you can do a lot with boolean logic, i.e. the art of combining ones and zeros to do more stuff!

 Here are three components you should know about:

 ### Inverter
 This one is very simple yet very important: If its input is low, its output is high and vice versa.

 ### And Gate
 An and gate has multiple inputs and one output, which is high if _every_ input is also high.

 ### Or Gate
 Or gates are similar to and gates but require just one of its inputs to be high for its output to be high as well.
 ...
 * Callout(Your Goal):
 Connect the LED to both inputs in such a way that it only lights up when both are high.
